...
class FlightAgent:
    ...
    def naturalize_result(self, raw_result: dict) -> str:
        prompt = ChatPromptTemplate.from_messages([
            ("system", "Sen travel asistani olarak kullaniciya tool sonucunu anlatacaksin."),
    ...
    def __call__(self, state: TravelState) -> TravelState:
        flights = [i for i in state["intents"] if i["domain"] == "flight"]
        for intent in flights:
            tool = get_mcp_tool("flight", intent["intent"])
            slots = {**intent["slots"], **state.get("filled_slots", {})}

            # eksik slot kontrol
            missing = validate_slots(tool, slots)
            if missing:
                state["missing_slots"] = {"flight": missing}
                return state

            # tool cagrisi
            result = tool.invoke(slots)
            state["results"]["flight"] = self.naturalize_result(result)
            state["missing_slots"] = {}
        return state
